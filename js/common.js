...
// ─────────────────────────── حصانة Clickjacking ───────────────────────────
//
// **بديلٌ أضعف من X-Frame-Options/frame-ancestors الحقيقيَّين لا استغناءً
// عنهما.** ذانك رأسا HTTP يمنعان تحميل الإطار من الأساس؛ هذا كودٌ يُنفَّذ
// بعد تحميل الصفحة، فيترك نافذةً قصيرة (جزءَ ثانية) يمكن خلالها لصفحة
// خبيثة أن تُطبِق طبقة فوق موقعنا قبل أن يُغلِق هذا السطر الإطار. سببه:
// Render لا يقرأ ملف `_headers` ولا يوجد وسم `<meta>` يكافئ هذين الرأسين
// (القيد موثَّق في CSP الصفحة — `frame-ancestors` غير مُفعَّل إلا عبر رأس
// HTTP فعلي، ولا يُطبَّق مطلقاً حين يصل عبر وسم). يبقى الإصلاح الكامل ضبط
// الرأسين من لوحة تحكّم الاستضافة مباشرة؛ هذا حدٌّ أدنى إلى أن يحدث ذلك.
